...
void HardFault_Handler(void)
{
  /* USER CODE BEGIN HardFault_IRQn 0 */
    ...
    volatile unsigned int dfsr = SCB->DFSR;
    ...
    if (hfsr & SCB_HFSR_FORCED_Msk) {
      // Forced Hard Fault - another fault has occurred but the corresponding handler was disabled or was not implemented
    }
    
    if (cfsr & 0xFFFF) {
      // Configurable Fault Status Register contains fault info
      if (cfsr & (0xFF << 8)) {
        // Bus Fault
      }
      if (cfsr & (0xFF << 16)) {
